        ...
        let wrongQuestionIds = new Set(); // 已在错题本中的题目 id
        let wrongQuestionsLoaded = false; // 是否已从后端加载过错题列表
        let categoryTree = {};
        ...
        // ==========================================
        //      WRONG QUESTION BOOK (错题本 · 艾宾浩斯遗忘曲线)
        // ==========================================

        // 同步编辑区"加入错题本"按钮状态（依赖 currentQuestionId）
        function refreshWrongBookBtn() {
            ...
            if (!wrongQuestionsLoaded) {
                fetch('/api/wrong-questions?filter=all')
                    .then(r => r.json())
                    .then(list => {
                        wrongQuestionIds = new Set((list || []).map(w => w.question_id));
                        wrongQuestionsLoaded = true;
                        // 使用最新的 currentQuestionId，避免异步期间用户切换了题目
                        updateWrongBookBtnUI(currentQuestionId);
                    })
                    .catch(() => {});
                return;
            }
            updateWrongBookBtnUI(qid);
        }

        function updateWrongBookBtnUI(qid) {
            const btn = document.getElementById('wrongBookBtn');
            const icon = document.getElementById('wrongBookBtnIcon');
            const text = document.getElementById('wrongBookBtnText');
            if (!btn || !icon || !text) return;
            if (!qid) {
                btn.classList.add('hidden');
                return;
            }
            const inBook = wrongQuestionIds.has(qid);
            btn.classList.remove('hidden');
            if (inBook) {
                icon.className = 'fa-solid fa-book-bookmark text-[10px]';
                text.textContent = '已在错题本';
                btn.classList.remove('text-amber-700', 'bg-amber-50', 'border-amber-200', 'hover:bg-amber-100');
                btn.classList.add('text-emerald-700', 'bg-emerald-50', 'border-emerald-200', 'hover:bg-emerald-100');
                btn.title = '点击移出错题本';
            } else {
                icon.className = 'fa-solid fa-book-medical text-[10px]';
                text.textContent = '加入错题本';
                btn.classList.remove('text-emerald-700', 'bg-emerald-50', 'border-emerald-200', 'hover:bg-emerald-100');
                btn.classList.add('text-amber-700', 'bg-amber-50', 'border-amber-200', 'hover:bg-amber-100');
                btn.title = '按艾宾浩斯遗忘曲线自动安排错题重做时间';
            }
        }
        ...
        function refreshWrongBadge() {
            fetch('/api/wrong-questions?filter=due')
                .then(r => r.json())
                .then(list => {
                    const dueCount = (list || []).length;
                    const badge = document.getElementById('wrongDueBadge');
                    if (badge) {
        ...
        function openSettingsModal() {
            ...
            const indicatorDot = document.querySelector('#settingsStatusIndicator span');
            if (indicatorDot) {
        ...
        function closeSettingsModal() {
            ...
            setTimeout(() => {
                ...
                const indicatorDot = document.querySelector('#settingsStatusIndicator span.bg-green-500');
                if (indicatorDot) {
        ...
        window.switchSettingsTab = function(tabName) {
            ...
            if (tabName === 'api') {
                ...
                btnUi.classList.remove('border-transparent', 'text-slate-500');
                btnUi.classList.add('border-brand-500', 'text-brand-600');
        ...
        function checkServiceStatus() {
            ...
            fetch('/api/questions')
                .then(r => r.json())
                .then(() => {
                    indicator.className = "inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-200";
                    indicator.innerHTML = '<span class="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse"></span><span>题库就绪</span>';
                })
                .catch(() => {
                    indicator.className = "inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-red-50 text-red-700 border border-red-200";
        ...
        // 题库侧边栏筛选面板展开/收起
        window.toggleBankFilters = function () {
            const panel = document.getElementById('bankFilterPanel');
            const chevron = document.getElementById('bankFilterChevron');
            if (!panel) return;
        ...
        // 题目预览: 全屏题目列表
        window.bankShowPreview = function () {
            showBankListView();
            if (typeof switchSidebarTab === 'function') switchSidebarTab('bank');
        };

        // 录入新题: 全屏进入编辑器
        window.bankStartNew = function () {
            showBankEditView();
            if (typeof checkAndSwitch === 'function') {
                checkAndSwitch(() => startNewQuestion());
            } else {
                startNewQuestion();
            }
        };
        ...
        // 题目筛选: 展开侧栏并展开筛选面板
        window.bankShowFilters = function () {
            showBankListView();
            const panel = document.getElementById('bankFilterPanel');
        ...
        //      题库工作台 · 单视图模式
        //      乔布斯式减法: 一个时刻只有一件事。
        //      'list' = 全屏题目列表; 'edit' = 全屏编辑器。
        // ==========================================
        let bankView = 'list';

        function applyBankView() {
            ...
            if (bankView === 'edit') {
                ...
                const backBtn = document.getElementById('bankBackToListBtn');
        ...
        // 全屏题目列表(预览)
        window.showBankListView = function () {
            bankView = 'list';
            applyBankView();
            // 切回列表时若有筛选展开则收起, 保持整洁
            const panel = document.getElementById('bankFilterPanel');
        ...
        // 全屏编辑器(录入/编辑)
        window.showBankEditView = function () {
            bankView = 'edit';
            applyBankView();
        };
        ...
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => { if (bankView === 'list') applyBankView(); }, 0);
        });
        ...
        window.selectWorkspace = function(workspaceId, workspaceName) {
            ...
            if (toggleSidebarBtn) {
                if (workspaceId === 'paper' || workspaceId === 'redo' || workspaceId === 'overview') {
            ...
            // 四工作台勾选状态同步：bank / paper / redo / overview
            const wsChecks = { bank: 'ws-check-bank', paper: 'ws-check-paper', redo: 'ws-check-redo', overview: 'ws-check-overview' };
            const wsBtns = { bank: 'ws-btn-bank', paper: 'ws-btn-paper', redo: 'ws-btn-redo', overview: 'ws-btn-overview' };
            Object.keys(wsChecks).forEach(key => {
                const check = document.getElementById(wsChecks[key]);
                if (check) check.classList.toggle('hidden', key !== workspaceId);
                const btn = document.getElementById(wsBtns[key]);
                if (btn) btn.classList.toggle('font-medium', key === workspaceId);
            });

            // 极简侧边导航高亮
            ['bank', 'paper', 'redo', 'overview'].forEach(key => {
                const navBtn = document.getElementById(`nav-${key}`);
                if (navBtn) navBtn.classList.toggle('active', key === workspaceId);
            });
