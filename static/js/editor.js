...
const PAGE_LIMIT = 20;
        ...
        function selectDraft(draft) {
            ...
            refreshWrongBookBtn(); // 草稿没有题目 id，隐藏"加入错题本"按钮

            // 单视图模式: 打开草稿即切到全屏编辑器
            if (typeof window.showBankEditView === 'function') {
                window.showBankEditView();
            }
        ...
        let loadQuestionsSeq = 0; // 请求序号: 防止过期响应覆盖最新渲染

        function loadQuestions(retryCount = 0) {
            const reqSeq = ++loadQuestionsSeq;
            ...
            fetch(`/api/questions?${params.toString()}`)
                ...
                .then(questions => {
                    // 过期响应直接丢弃(已有更新的请求)
                    if (reqSeq !== loadQuestionsSeq) return;
                    qListContainer.innerHTML = '';
                    ...
                    pageItems.forEach(item => {
                        ...
                        // Render KaTeX inline for this card (仅当内容包含公式时,避免无谓开销)
        ...
        function parseMarkdownWithMath(text) {
            ...
            // 4.5 裸 LaTeX 数学环境(未用 $ 包裹的 \begin{aligned} 等)自动包裹为 $$...$$,保证 KaTeX 渲染
            // 注意: 替换串中 $$ 会被 JS 解释为单个 $,必须使用函数形式返回字面量 $$
            // 先处理 KaTeX 不支持的 align/equation/gather 等多行环境: 转换为 aligned
            tempText = tempText.replace(/\\begin\{(align\*?|equation\*?|gather\*?|flalign\*?|multline\*?)\}([\s\S]*?)\\end\{\1\}/g, function(m, env, inner) {
                return '$$\\begin{aligned}' + inner + '\\end{aligned}$$';
            });
            // 其余 KaTeX 支持的数学环境直接包裹
            tempText = tempText.replace(/\\begin\{(aligned|alignedat|split|gathered\*?|cases|dcases|rcases|matrix|pmatrix|bmatrix|vmatrix|Vmatrix|array|smallmatrix|subarray|cd|Bmatrix)\}([\s\S]*?)\\end\{\1\}/g, function(m, env, inner) {
                return '$$\\begin{' + env + '}' + inner + '\\end{' + env + '}$$';
            });
            // 上述包裹后的 $$...$$ 块立即保护,避免被后续正则干扰
            tempText = tempText.replace(/\$\$([\s\S]*?)\$\$/g, savePlaceholder);
        ...
        setInterval(() => {
            fetch('/api/heartbeat', { method: 'POST' }).catch(() => {});        }, 15000);

        // 🟢 KaTeX 预热：页面加载后空闲时提前初始化渲染引擎（字体/样式），
        // 避免首次渲染题目卡片时产生可见卡顿
